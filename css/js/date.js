 ...
 * 
 * Thy Add in: daysUpToMonth, daysUpToMonthLeapYear
 */
Date.CultureInfo = {
    name: "en-US",
...
/* Thy add in -- for aging subroute */
Date.getDifferenceInDays = function(d1, d2){
	if(d1.compareTo(d2) <= 0){ 	
		return d2.getDayOfYear()-d1.getDayOfYear()+365*(d2.getYear() - d1.getYear());
	}
};
Date.getDifferenceInMonthsAndDays = function(d1, d2){
	if(d1.compareTo(d2) <= 0){ 
		var from = d1.clone();
		var to = d2.clone();
		if(from.getDate() <= to.getDate()){
			var months = (to.getMonth()-from.getMonth()+12)%(12*(to.getYear()-from.getYear()+1));
			from.setYear(to.getYear());
			from.setMonth(to.getMonth());
			return {
				months: months,
				days: Date.getDifferenceInDays(from, to)
			};
		}else{
			var days = Date.getDifferenceInDays(from, to);
